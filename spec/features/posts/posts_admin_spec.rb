...
feature 'Admin Post Privileges', :type => :feature do

  before do
    @user = User.create(email: "maily@email.com", password: "passywordy", role: 'admin')
  ...
  it 'can delete posts belonging to other users' do
    new_user = User.create(email: "nonadmin@mail.com", password: "not an admin")
    post = Post.create(content: "some post content", user_id: new_user.id)
    visit post_path(post)
    click_button "Delete"
